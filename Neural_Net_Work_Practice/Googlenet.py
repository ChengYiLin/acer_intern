...
def Inception(x, nb_filter_para):
    ...
    branch5x5 = Conv2D(branch3[1],(5,5), padding='same',strides=(1,1),name=None)(branch5x5)
...
InceptionV1_model.fit_generator(img_gen.flow(X_train*255, y_train, batch_size = 16),steps_per_epoch = len(X_train)/16, validation_data = (X_test,y_test), epochs = 30 )

# Save the model
InceptionV1_model.save('model/Googlenet_IV1.h5')

# evaluate the model
scores = InceptionV1_model.evaluate(X_test, y_test, verbose=0)
print("%s: %.2f%%" % (InceptionV1_model.metrics_names[1], scores[1]*100))